...
def train(args, data_cfg, model_cfg, generator, optimizer, train_dataloader, DPG_edge_index, DPG_edge_type):
    ...
    for diags, procs, drugs, len_list in train_dataloader:
        ...
        loss_relation = loss_AG(generator.edge_predict_net(edge_embs), DPG_edge_type)
        if args.reinforce == 'no':
            l = loss + args.alpha * loss_relation
        else:
            l = loss + args.beta * loss_relation
        l.backward()
        optimizer.step()
...
def run(args, data_cfg, model_cfg):
    ...
    # print(DPG_edge_type)
    # print(DPG_edge_type.size())
    # print(DPG_edge_type.sum())
    # exit(0)
    # 构建dataloader
    train_dataloader = Data.DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True)
    # 设置模型和优化器
    if args.reinforce == 'no':
        generator = DPG(data_cfg, model_cfg).to(model_cfg.device)
    else:
        # 读取预训练的模型
        model_root = data_cfg.result_root + 'DPG/results/models/'
        model_path = model_root + 'MLE' + '_{}.pkl'.format(args.alpha)
    ...
    # optimizer = torch.optim.(params=generator.parameters(), lr=args.lr)
    best_F = 0
    for epoch in tqdm(range(args.num_epochs)):
        if epoch % args.eval_num == 0:
            ...
            if args.save == 'yes':
                if args.reinforce == 'no':
                    utils.save(generator, data_cfg, result_val, result_val['F'] > best_F, epoch, 'MLE', args.alpha, args.beta, args.reinforce, args.lr)
                else:
                    utils.save(generator, data_cfg, result_val, result_val['F'] > best_F, epoch, 'RL', args.alpha, args.beta, args.reinforce, args.lr)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--train", type=str, default="yes", help="train the generator or not")
    parser.add_argument("--test", type=str, default="yes", help="test the generator or not")
    parser.add_argument("--reinforce", type=str, default="no", help="use reinforcement learning or not")
    parser.add_argument("--alpha", type=float, default=1.0, help="contribution ratio of loss_relaiton in MLE")
    parser.add_argument("--beta", type=float, default=1.0, help="contribution ratio of loss_relaiton in RL")
    ...
    if args.test == 'yes':
        if args.reinforce == 'no':
            utils.performance_test(args, data_cfg, model_cfg, 'MLE', args.alpha, args.beta, args.reinforce, args.lr)
        else:
            utils.performance_test(args, data_cfg, model_cfg, 'RL', args.alpha, args.beta, args.reinforce, args.lr)